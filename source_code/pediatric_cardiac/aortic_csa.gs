...
// ─────── HELPER FUNCTIONS FOR LMS CALCULATIONS ────────
 ...
 */
function handleRequest_peds_aorta_csa(e, SPREADSHEET_ID) {
  const SHEET_NAME = 'pediatric_cardia.aortic_csa';

  // --- 1. Extract and Validate Parameters ---
  const { parameter, gender, level, measured, age } = e.parameter;

  if (!parameter || !gender || !level || !measured || !age) {
    throw new BadRequestError('Missing one or more required parameters: parameter, gender, level, measured, age.');
  }
  ...
  // Use Math.floor for integer age matching, allows for ages like 1.5 to match age 1 row
  ...
  for (const row of rows) {
    ...
    const rowLevel = normalize(row[headers.indexOf('level')]);

    if (
      rowParameter === normalizedParameter &&
      rowGender === normalizedGender &&
      rowLevel === normalizedLevel &&
  ...
  if (!foundRowData) {
    throw new NotFoundError(`No reference data found for parameter='${parameter}', gender='${gender}', level='${level}', and age='${age}'.`);
  }
